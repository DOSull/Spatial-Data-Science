...
globals [
  gliders ;; a list of glider cell coordinates
]

patches-own [
  next-state ; the next state of the patch
  state ; current state of the patch
]
...
to setup
  clear-all
  ;; r:setPlotDevice
  ;; there are four possible gliders corresponding to
  ;; different directions of travel
  set gliders (list (list [-1 1] [0 1] [1 1]  ;; NE
                                       [1 0]
                                 [0 -1])
                    (list [-1 1] [0 1] [1 1]  ;; NW
                          [-1 0]
...
;; patch updating code
;; note that all next-states have to be determined
;; before any are changed i.e. synchronous update
to update-states
  ask patches [
    set next-state 0; assume all patches will die
    ; count how many live neighbours of the patch
    let n sum [state] of neighbors

    ifelse state = 1 [ ; patch is alive
      ; stays alive with 2 or 3 live neighbours
...
;; randomly make a glider at an empty spot
to make-glider
  ;; find an empty spot
...
Also in this model is that cell states are stored as integer 0 ('dead') or 1 ('live') values.  This means it is easy to count live cells in neighbourhoods using the `sum` reporter.  It is also requires the `update-states` procedure to colur patches according to their current state.
