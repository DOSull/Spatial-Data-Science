...
```{r}
basemap <- get_tiles(abb, zoom = 10, crop = TRUE)
tm_shape(basemap) + 
...
Other basemaps are available than OpenStreetMap. You can see the options in the `get_tiles()` function help. For example, this is a nice one (if a bit distracting). Some of them provide no label and only label options, so you can this sort of thing:

```{r}
basemap <- get_tiles(abb %>% st_buffer(5000), 
                     provider = "Stamen.TerrainBackground", crop = TRUE, zoom = 10)
labels <- get_tiles(abb %>% st_buffer(5000), 
                    provider = "Stamen.TerrainLabels", crop = TRUE, zoom = 10)
